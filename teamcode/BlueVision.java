...
public class BlueVision extends LinearOpMode {
    ...
    private void visionAutonomy() {
        if (Left) {
            ...
            AutonomyMove(400, 0.39, 0.73);
        }
        else if (Mid) {
            telemetry.addData("Mid", true);
            AutonomyMove(730, 0.40, 0.71);
        }
        else if (Right) {
            telemetry.addData("Up", true);
            AutonomyMove(1200, 0.49, 0.85);
        }
    }
    ...
    private void AutonomyMove(int handHeight, double forwardToTower, double backwardToParking) {
        SetDrivePower(1);
        Forward(0.1);
        TurnRight(92);
        sleep(100);
